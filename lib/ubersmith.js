...
var populateUberData = function(params)
{
  ...
  var saveValue = function(key, value, serialize, expires)
  {
  ...
  var _handleClient = function(client)
  {
    ...
    {
      saveValue('US:client:' + clientID, client);
  ...
  var _handleSingleDevice = function(device)
  {
    ...
    saveValue('US:devices:' + deviceID, JSONDevice, false);

    if (hostname && hostname != 'null')
    {
      saveValue('US:devices:hostname:' + hostname + '.contegix.mgmt', JSONDevice, false);
    }
  };
  ...
  var _handleDevice = function(device)
  {
    ...
    {
      addValuetoSet('US:devices:locations:racks:' + device.rack_code, JSONDevice, false);
    }

    if (device.fac_id && device.fac_id != 'null')
    {
      addValuetoSet('US:devices:locations:facility:' + device.fac_id, JSONDevice, false);
      addValuetoSet('US:devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id, JSONDevice, false);
      addValuetoSet('US:devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id, JSONDevice, false);
      addValuetoSet('US:devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id + ':row:' + device.row_id, JSONDevice, false);
      addValuetoSet('US:devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id + ':row:' + device.row_id + ':rack:' + device.rack_id, JSONDevice, false);
      addValuetoSet('US:devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id + ':row:' + device.row_id + ':rack:' + device.rack_id + ':position:' + device.rack_pos, JSONDevice, false);
    }

    if (device.clientid && device.clientid != 'null')
    {
      addValuetoSet('US:devices:clientid:' + device.clientid, JSONDevice, false);
      addValuetoSet('US:client:' + device.clientid + ':devices', JSONDevice, false);
    }
  };

  var _handleContact = function(contact)
  {
    addValuetoSet('US:client:' + contact.client_id + ':contacts', JSON.stringify(contact), false);
    saveValue('US:contacts:' + contact.contact_id, JSON.stringify(contact), false);
  };
  ...
  var _handleTicket = function(ticket)
  {
    redisClient.get('devices:deviceid:' + ticket.device_id, function (err, reply)
    {
      ...
      addValuetoSet('US:client:' + ticket.client_id + ':tickets', JSONTicket, false);
      addValuetoSet('US:devices:' + ticket.device_id + ':tickets', JSONTicket, false);
      saveValue('US:tickets:' + ticketID, JSONTicket, false);
    });
  };
  ...
  var _handleFacility = function(facility)
  {
    ...
    saveValue('US:locations:facility:' + facilityID + ':name', facilityName);
    saveValue('US:locations:facility:' + facilityID + ':code', facilityCode);
    addValuetoSet('US:locations:facilities', { id: facilityID, name: facilityName, code: facilityCode });
    ...
    Object.keys(zoneList).forEach(function (zoneID)
    {
      ...
      saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':name', zoneName);
      saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':code', zoneCode);
      addValuetoSet('US:locations:facility:' + facilityID + ':zones', { id: zoneID, name: zoneName, code: zoneCode });
      ...
      Object.keys(cageList).forEach(function (cageID)
      {
        ...
        saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':name', cageName);
        saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':code', cageCode);
        addValuetoSet('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cages', { id: cageID, name: cageName, code: cageCode });
        ...
        Object.keys(rowList).forEach(function (rowID)
        {
          ...
          saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':row:' + rowID + ':name', rowName);
          saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':row:' + rowID + ':code', rowCode);
          addValuetoSet('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':rows', { id: rowID, name: rowName, code: rowCode });
          ...
          Object.keys(rackList).forEach(function (rackID)
          {
            ...
            saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':row:' + rowID + ':rack:' + rackID + ':name', rackName);
            saveValue('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':row:' + rowID + ':rack:' + rackID + ':code', rackCode);
            addValuetoSet('US:locations:facility:' + facilityID + ':zone:' + zoneID + ':cage:' + cageID + ':row:' + rowID + ':racks', { id: rackID, name: rackName, code: rackCode });
          });
        });
      });
    });
  ...
  var _handleDeviceGroupList = function(deviceGroupList)
  {
    ...
    redisClient.del('devices:type_group:' + devtype_group_id, function(err, reply) {
      ...
      _.each(deviceGroupList, function(device) {
        ...
        addValuetoSet('US:devices:type_group:' + devtype_group_id, JSON.stringify(device), false);
      })
    });
  }
  ...
  var _handleTypeList = function(typeList) {
    ...
    saveValue('US:device_types:list', typeList);
  };
  ...
  UberettaStone['devices']['deviceID'] = {
    redis: { key: 'US:devices:%token%', method: 'get'},
  ...
  UberettaStone['devices']['hostname'] = {
    redis: { key: 'US:devices:hostname:%token%', method: 'get'},
    ubersmith: null
  };
  UberettaStone['devices']['rack'] = {
    redis: { key: 'US:devices:locations:racks:%token', method: 'smembers'},
    ubersmith: null
  };

  UberettaStone['devices']['type_group'] = {
    redis: { key: 'US:devices:type_group:%token%', method: 'smembers'},
