...
module.exports = function(params)
{
  ...
  var _handleClient = function(client, callback)
  {
    ...
    {
      addValuetoSet('clients', clientID);
  ...
  var _handleClientTickets = function(ticketList)
  {
    ...
    redisClient.del('clients:' + clientID + ':tickets', function (err, reply) {
      ...
      _.each(tickets, function (ticket) {
        ...
        addValuetoSet('clients:' + clientID + ':tickets', ticketID);
      });
    });
  };
  ...
  var _handleDeviceSets = function(device, callback)
  {
    ...
    addValuetoSet('devices:list', deviceID, false);
    addValuetoSet('devices:type_group:' + device.devtype_group_id, deviceID, false);
    addValuetoSet('devices:type:' + device.type_id, deviceID, false);

    if (device.rack_code && device.rack_code != 'null')
    {
      addValuetoSet('devices:locations:racks:' + device.rack_code, deviceID, false);
    }

    if (device.fac_id && device.fac_id != 'null')
    {
      addValuetoSet('devices:locations:facility:' + device.fac_id, deviceID, false);
      addValuetoSet('devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id, deviceID, false);
      addValuetoSet('devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id, deviceID, false);
      addValuetoSet('devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id + ':row:' + device.row_id, deviceID, false);
      addValuetoSet('devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id + ':row:' + device.row_id + ':rack:' + device.rack_id, deviceID, false);
      addValuetoSet('devices:locations:facility:' + device.fac_id + ':zone:' + device.zone_id + ':cage:' + device.cage_id + ':row:' + device.row_id + ':rack:' + device.rack_id + ':position:' + device.rack_pos, deviceID, false);
    }

    if (device.clientid && device.clientid != 'null')
    {
      addValuetoSet('devices:clientid:' + device.clientid, deviceID, false);
      addValuetoSet('clients:' + device.clientid + ':devices', deviceID, false);
    }
  ...
  {
    addValuetoSet('clients:' + contact.client_id + ':contacts', contact.contact_id, false);
  ...
  var _handleTicket = function(ticket)
  {
    ...
    getItem('devices', 'deviceID', deviceID, function (err, device){
      ...
      addValuetoSet('clients:' + clientID + ':tickets', ticketID, false);
      addValuetoSet('devices:' + deviceID + ':tickets', ticketID, false);
      addValuetoSet('tickets:list', ticketID, false);
    });
  };
  ...
  var _handleFacility = function(facility)
  {
    ...
    saveValue('locations:facilities:facilityID:' + facilityID, facility);
  ...
  var _handleDeviceGroupList = function(deviceGroupList)
  {
    ...
    redisClient.del('devices:type_group:' + devtype_group_id, function(err, reply) {
      ...
      _.each(deviceGroupList, function(device) {
        var deviceID = device.dev;
        addValuetoSet('devices:type_group:' + devtype_group_id, deviceID, false);
      })
    });
  }
...
// from Redis and Ubersmith
    ...
    if (key == '')
    {
      ...
      redisClient.get(key, function (err, reply){
        if (err)
        {
          ...
          if (reply != '[]' && reply != [] && reply != new Array() && reply != '' && reply != null)
          {
            try {
              if (method == 'smembers')
              {
                async.map(reply, function(deviceID, aSyncMapCallback) {
                  getItemFromRedis('devices:deviceID:' + deviceID, 'get', function(err, device) {
                    if (err) {
  ...
  var getSingleItemFromRedis = function(key, callback)
  {

  }

  var getItemListFromRedis= function(key, callback)
  {
    redisClient.get(key, function (err, reply)
    {
      if (err) {
...
// is emitted to the item handlers
  ...
  Emitter.on('configure.complete', function(params) {
    deviceProvider = require('deviceProvider')(redisClient, logger, '');
