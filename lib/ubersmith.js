...
// from Redis and Ubersmith
  ...
  UberettaStone['uber']['metadata_group_list'] = {
    redis: { key: 'ubersmith:metadata_group_list', method: 'smembers'},
    ubersmith: { method: 'uber.metadata_group_list', params: '&meta_type=%token%',  key: 'uber.quick_stats'}
  };

  UberettaStone['uber']['metadata_field_list'] = {
    redis: { key: 'ubersmith:metadata_field_list', method: 'smembers'},
    ubersmith: { method: 'uber.metadata_field_list', params: '&meta_type=%token%',  key: 'uber.quick_stats'}
  };
  ...
  var submitNewLead = function(postData, callback) {
  ...
  var getAllClients = function (getClientCallback)
  {
    getCachedItem('clients', 'listAll', '', function(err, clientList) {
      if (err) {
        getClientCallback(err);
      } else {
        parseClientList(clientList, getClientCallback);
      }
    });
  };

  var getMetadataFields = function (group, getMetadataCallback)
  {
    getCachedItem('uber', 'metadata_field_list', group, getMetadataCallback);
  };

  var getMetadataGroup = function (group, getMetadataCallback)
  {
    getCachedItem('uber', 'metadata_group_list', group, getMetadataCallback);
  };
  ...
  var getLeads = function (getLeadCallback)
  {
    getCachedItem('leads', 'list', '', function (err, leadList) {
      if (err) {
        ...
        parseClientList(leadList, getLeadCallback);
      }
    });
  };

  var parseClientList = function (clientList, done) {
    getAdmins(function(err, adminList){
      if (err) {
        done(err);
      } else {
        ...
        _.each(clientList, function(client) {
          client.salesperson_name = '';
          client.salesperson_email = '';
          if (client.hasOwnProperty('salesperson') && client.salesperson != '0' && adminList.hasOwnProperty(client.salesperson))
          {
            var salesPerson = adminList[client.salesperson];
            if (salesPerson.hasOwnProperty('name')) {
              client.salesperson_name = salesPerson.name;
            }
            if (salesPerson.hasOwnProperty('email')) {
              client.salesperson_email = salesPerson.email;
            }
          }
          retClientList[client.clientid] = client;
        });
        done(null, retClientList);
      }
    });
  };
