...
module.exports = function(params)
{
  ...
  var _handleClient = function(client)
  {
    ...
    if (client.company != 'REMOVED' && client.active == 1)
    {
      ...
      Emitter.emit('client.ready_for_contacts', 'client.contact_list', '&client_id=' + clientID + '&metadata=1&contact_active=1', 'contact.list');
    }
    else
    {
...
// from Redis and Ubersmith
  ...
  UberettaStone['devices']['list'] = {
    redis: { key: 'devices', method: 'smembers'},
  ...
  var getDeviceHostnames = function(callback)
  {
    getItem('devices', 'list', '', function (err, deviceList) {
      ...
      _.each(deviceList, function(device) {
        ...
        {
          _.defaults(device, {first: '', last: '', email: '', listed_company: '', email: '', clientid: '', location: '', metadata: { runbook_instructions_device: '' }});
          deviceHostnames[device.dev_desc + '.contegix.mgmt'] = {dev_desc: device.dev_desc + '.contegix.mgmt', dev: device.dev, full_name: device.first + ' ' + device.last, email: device.email, company: device.listed_company, clientid: device.clientid, location: device.location, runbook: device.metadata.runbook_instructions_device };
        }
      });
...
// emits "ready.*.list" indicating the data from the key is ready
  ...
  Emitter.on('ready.contact.list', _handleContactList);
...
//Emitter.on('new.device', _handleDevice);
  Emitter.on('new.client', _handleClient);
