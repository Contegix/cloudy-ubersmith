...
//var redis_cache = cache_manager.caching({store: redis_store, db: 9, ttl: 30/*seconds*/});
var cache_primary = cache_manager.caching({store: 'memory', max: 256000 /*Bytes*/, ttl: 30 /*seconds*/});
...
module.exports = function(params)
{
  ...
  var start = function(params)
  {
    ...
    logger.log('verbose', 'Cache Warmup Started', {params: params});
  ...
  */
  ...
  var getValue = function (key, callback) {
    ...
    {
      logger.log('verbose', 'Redis Store Disabled', {key: key});
  ...
  {
    logger.log('debug', 'Setting Expiration', {key: key, expiration: expiration});
      ...
      redisClient.sadd(key
        ...
        , function (err, res) {
          if (err)
          {
            ...
            //logger.log('verbose', 'Saved value for set: ' + key, {key: key, res: res});
          }
        }
      );
    }
...
*/
  ...
  var _handleSingleDevice = function(device)
  {
    ...
    logger.log('verbose', 'Saving single device', { deviceID: deviceID, hostname: hostname })
      ...
      _.each(clientList, function(client) {
        ...
        async.eachLimit(list, 15
          , function(client, clientListCallback) {
            ...
            logger.log('verbose', 'finished handling client', {});
  ...
  var _handleDeviceList = function(deviceList)
  {
    ...
    async.eachLimit(list, 15
      , function(device, deviceListCallback) {
        ...
        // When all devices are finished, let others know.
        logger.log('verbose', 'finished handling devices', {});
...
// from Redis and Ubersmith
  ...
  var ubersmithRequest = function(uberOptions, ubersmithRequestCallback) {
    cache_primary.wrap(uberOptions.url, function(ubersmithRequestCallback) {
      var request = require('request');
      ...
      async.waterfall([
        function(waterfallCallback) {
          if (translationTable.redis.method == 'get') {
            getItemFromRedis(replaceToken(translationTable.redis.key, value)
              , function (err, reply) {
                waterfallCallback(reply, err);
              }
            );
          } else {
            getItemListFromRedis(replaceToken(translationTable.redis.key, value)
              , function (err, reply) {
                waterfallCallback(reply, err);
              }
            );
        ...
        function(redisErr, waterfallCallback) {
          getItemFromUbersmith(translationTable.ubersmith.method, replaceToken(translationTable.ubersmith.params, value), translationTable.ubersmith.key
            , function (err, reply) {
              waterfallCallback(reply, err);
            });
        }
        ...
        if (item) {
          getItemCallback(null, item);
        } else {
          getItemCallback(new Error('Could not locate Item'), null);
        }
      });
    } else {
