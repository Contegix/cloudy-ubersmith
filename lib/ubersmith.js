  ...
  var getDeviceHostnames = function(callback)
  {
    getItem('devices', 'list', '', function (err, deviceList) {
      ...
      _.each(deviceList, function(device) {
        ...
        {
          _.defaults(device, {first: '', last: '', email: '', listed_company: '', email: '', location: '', metadata: { runbook_instructions_device: '' }});
          deviceHostnames[device.dev_desc + '.contegix.mgmt'] = {dev_desc: device.dev_desc + '.contegix.mgmt', dev: device.dev, full_name: device.first + ' ' + device.last, email: device.email, company: device.listed_company, location: device.location, runbook: device.metadata.runbook_instructions_device };
        }
      });
