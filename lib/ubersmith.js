...
// from Redis and Ubersmith
        ...
        function(redisResponse, waterfallCallback) {
          if (redisResponse != 'MISS')
          {
            ...
            if (!translationTable.ubersmith) {
              ...
              waterfallCallback(new Error('failed to get item from ubersmith'), 'MISS');
            } else {
  ...
  var authenticateUser = function(user, pass, authenticateCallback) {
    getItemFromUbersmith('uber.check_login', '&user=' +  encodeURI(user) + '&pass=' + encodeURI(pass), 'uber.check_login', authenticateCallback);
  }
  ...
  var getDeviceByHostname = function(hostname, getDeviceHostnameCallback)
  {
    ...
    getCachedItem('devices', 'hostname', hostname, function (err, reply) {
      if (reply == 'MISS') {
        logger.log('debug', 'Could not get device by hostname from cache');
        getCachedItem('devices', 'list', '', function (err, deviceList) {
          if (err) {
            getDeviceHostnameCallback(err, null);
          } else {
            logger.log('debug', 'Got Device list');
            ...
            var device = _.findWhere(deviceList, {dev_desc: shortHostname});
            if (device && device != '' && device.dev) {
              var deviceID = device.dev;
              logger.log('debug', 'located deviceID from list', {deviceID: deviceID, dev_desc: shortHostname});
              getDeviceByID(deviceID, getDeviceHostnameCallback);
            } else {
              logger.log('error', 'a device with this shortname wasnt found', {shortHostname: shortHostname});
              getDeviceHostnameCallback(new Error('could not retrieve device by name'), null);
            }
          }
        });
      } else {
